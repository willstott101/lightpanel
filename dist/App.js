...
})({"9lo96":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "69f74e7f31319ffd";
module.bundle.HMR_BUNDLE_ID = "6810e1288834faca";
"use strict";
...
},{}],"3Ttui":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/* frontend/App.svelte generated by Svelte v3.42.3 */ var _internal = require("svelte/internal");
function create_fragment(ctx) {
    let h1;
    return {
        c () {
            h1 = _internal.element("h1");
            h1.textContent = `Hello ${name}!`;
        },
        m (target, anchor) {
            _internal.insert(target, h1, anchor);
        },
        p: _internal.noop,
        i: _internal.noop,
        o: _internal.noop,
        d (detaching) {
            if (detaching) _internal.detach(h1);
        }
    };
}
let name = 'world';
class App extends _internal.SvelteComponent {
    constructor(options){
        super();
        _internal.init(this, options, null, create_fragment, _internal.safe_not_equal, {
        });
    }
}
exports.default = App;

},{"svelte/internal":"hruTl","@parcel/transformer-js/src/esmodule-helpers.js":"JacNc"}],"hruTl":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function init_hydrate(target) {
    ...
    */ // Compute longest increasing subsequence
    ...
    for(let i = 0; i < children.length; i++){
        ...
        const seqLen = (longest > 0 && children[m[longest]].claim_order <= current ? longest + 1 : upper_bound(1, longest, (idx)=>children[m[idx]].claim_order
        , current)) - 1;
        p[i] = m[seqLen] + 1;
...
function attr1(node, attribute, value) {
...
function set_attributes(node, attributes) {
    ...
    for(const key in attributes){
        ...
        else attr1(node, key, attributes[key]);
    }
}
function set_svg_attributes(node, attributes) {
    for(const key in attributes)attr1(node, key, attributes[key]);
}
function set_custom_element_data(node, prop, value) {
    if (prop in node) node[prop] = typeof node[prop] === 'boolean' && value === '' ? true : value;
    else attr1(node, prop, value);
}
...
function claim_node(nodes, predicate, processNode, createNode, dontUpdateLastIndex = false) {
    ...
    const resultNode = (()=>{
        ...
        for(let i1 = nodes.claim_info.last_index - 1; i1 >= 0; i1--){
            ...
            if (predicate(node)) {
                const replacement = processNode(node);
                if (replacement === undefined) nodes.splice(i1, 1);
                else nodes[i1] = replacement;
...
function claim_html_tag(nodes) {
    ...
    if (start_index === end_index) return new HtmlTagHydration();
    init_claim_info(nodes);
...
function toggle_class(element1, name, toggle) {
    element1.classList[toggle ? 'add' : 'remove'](name);
}
...
function get_custom_elements_slots(element1) {
    const result = {
    };
    element1.childNodes.forEach((node)=>{
...
function create_animation(node, from, fn, params) {
    ...
    loop((now1)=>{
        if (!started && now1 >= start_time) started = true;
        if (started && now1 >= end) {
            tick(1, 0);
            stop();
        }
...
function create_in_transition(node, fn, params) {
    ...
    function go() {
        const { delay =0 , duration =300 , easing =identity , tick: tick1 = noop , css  } = config || null_transition;
        ...
        task = loop((now1)=>{
            if (running) {
                ...
                if (now1 >= start_time) {
                    const t = easing((now1 - start_time) / duration);
                    tick1(t, 1 - t);
                }
            }
            return running;
...
function create_bidirectional_transition(node, fn, params, intro) {
    ...
    function go(b) {
        ...
        if (running_program || pending_program) pending_program = program;
            ...
            loop((now1)=>{
                if (pending_program && now1 > pending_program.start) {
                ...
                if (running_program) {
                    if (now1 >= running_program.end) {
                        tick1(t = running_program.b, 1 - t);
                    ...
                    } else if (now1 >= running_program.start) {
                        const p = now1 - running_program.start;
...
function handle_promise(promise1, info) {
    ...
    function update1(type, index, key, value) {
        ...
        if (info.block) {
            if (info.blocks) info.blocks.forEach((block1, i)=>{
                if (i !== index && block1) {
                    group_outros();
                    transition_out(block1, 1, 1, ()=>{
                        if (info.blocks[i] === block1) info.blocks[i] = null;
                    });
                    check_outros();
                }
            });
...
function create_ssr_component(fn) {
    ...
    return {
        render: (props = {
            ...
            const html = $$render(result, props, {
            }, $$slots, context);
            run_all(on_destroy);
            return {
                html,
...
function set_data_dev(text1, data) {
    data = '' + data;
    if (text1.wholeText === data) return;
...
},{}]},["9lo96","3Ttui"], "3Ttui", "parcelRequired298")
